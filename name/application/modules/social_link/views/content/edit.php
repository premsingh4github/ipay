	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<?php // Change the values in this array to populate your dropdown as required
				...
				echo form_dropdown('social_link_status', $options, set_value('social_link_status', isset($social_link['status']) ? $social_link['status'] : ''), 'Status');
			?>

			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('social_link_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/content/social_link', lang('social_link_cancel'), 'class="btn btn-warning"'); ?>
				
			<?php if ($this->auth->has_permission('Social_Link.Content.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('social_link_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('social_link_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
